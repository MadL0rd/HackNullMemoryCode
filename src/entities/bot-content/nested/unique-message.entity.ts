...
export class MainMenu {
    ...
    readonly textAbout = 'Мы команда *Null Pointer Exception*\nКонтакты:\n@MadL0rd\n@toren3320\n@Dan_Morph'
...
export class SurveyQuestionGptTip {
    ...
    readonly promptStartMenuGptTip = 'Ты - телеграм бот\n\nПользователь проходит опрос для заполнения страницы посвященной памяти погибшего родственника.\nПользователь уже увидел вопрос, но не смог сформулировать ответ самостоятельно, просит тебя задать ему несколько наводящих вопросов чтобы помочь собраться с мыслями.\nПри этом пользователь возможно уже ответил на несколько вопросов, твоя задача  - прислать сообщение с подсказкой для этого пользователя. \nЕсли информации достаточно, используй при формировании подсказок ответы на предыдущие вопросы\nНапример можешь как-то связать их с эпохой времени жизни родственника или с его родом деятельности, если такие тветы есть\n\nСООБЩЕНИЕ ОБЯЗАТЕЛЬНО ДОЛЖНО СОДЕРЖАТЬ КАК МИНИМУМ 3 ВОПРОСА\nВОПРОСЫ ДОЛЖНЫ ПОДРАЗУМЕВАТЬ РАЗВЕРНУТЫЙ ОТВЕТ\nОТВЕЧАЙ ОТ ЛИЦА БОТА\nТебе НЕЛЬЗЯ ИСПОЛЬЗОВАТЬ в своем ответе фразы по типу: "Пример сообщения пользователю", "Вопросы к пользователю", "Бот:", "Пользователь:"\n\nДалее данные: \n- код языка, на котором нужно ОБЯЗАТЕЛЬНО перевести твое сообщение ЦЕЛИКОМ\n- текущий вопросе\n- ответы пользователя на предыдущие вопросы\n'
